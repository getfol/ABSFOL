...
;; adding a module to the system: the following function takes as
;; argument the name of a module, compile its files, load them and
;; install the new command (providing the configuration file is
;; correct
(DEFUN ADD-MODULE (module)
 (PROGN
  (INSTALL-MODULE
   (MODULE-GET-FILES  module) 
   (MODULE-GET-SRCDIR module)
   (MODULE-GET-OBJDIR module)
   (IF (EQ (MODULE-GET-MODE module) 'INTERPRETED)
           'INTERPRETED
           (SYS-GET-MODE (SYSMASTER-GET))))
  (SUBR-RESTORE)
  (TERPRI)
  (TERPRI)
  (PRINC "Master system: ")
  (PRINC (SYS-GET-NAME (SYSMASTER-GET)))
  (TERPRI)
  (PRINC "Module:        ")
  (PRINC (MODULE-GET-NAME module))
  (TERPRI)
  (PRINC "Status:        *")
  (PRINC (IF (EQ (MODULE-GET-MODE module) 'INTERPRETED)
                         'INTERPRETED
                         (SYS-GET-MODE (SYSMASTER-GET))))
  (PRINC "*")
  (TERPRI)
